"""Test full Perception Pipeline combining MapROIExtractor, CharacterDetector, and NMS on base rings."""
...
def run_pipeline_test():
    ...
    for label, path in img_files:
        ...
        # 2. Check if in Placement Phase (if button "ELEGIR LOS RETOS" is visible or placement tiles exist)
        # In placement phase, require higher body edge density (>= 0.08) and sat_std (>= 35.0) to filter empty tiles
        ...
        for c in cnts:
            ...
            if 16 <= bw <= 80 and 8 <= bh <= 50 and 0.25 <= aspect <= 0.95:
                ...
                # Reject solid empty placement phase tiles
        ...
        print(f"Final Perception Detections: {len(detections)}")
        for d in detections:
        ...
        cv2.imwrite(f"debug_captures/scratch/pipe_final_{label.split()[1]}.png", overlay)
